...
($trash, $srh) = split(/=/,$input[0]);

$srh =~ s/\+/ /g;
$srh =~ s/%(..)/pack("c",hex($1))/ge;

if ($srh eq "") { &blank_entry; }
...
foreach $fname (@files)
{
 open (SERCH, $bootdir.'/'.$fname) if (($fname ne ".") && ($fname ne ".."));
 if (($fname ne "bootlegs.html") && !($fname =~ /gif/)) {

 while (<SERCH>)
  {
   s/<.*?>//g;
   if (/$srh/i) { push @results, $fname; }
  }
 }
 close SERCH;
}

open TITLES, $bootdir.'/'."bootlegs.html";
while (<TITLES>)
{
foreach $s (@results) {
 if (/$s/) {
  ...
  s/\s*//;
  s/(&nbsp;)//g;
  push @code, qq*<a href="../bootleg/$s">$_</a><br>\n*;
 }
 }
}
close TITLES;
...
<HEAD>
   <TITLE>Bootlegs</TITLE>
         ...
         <CENTER><P><IMG SRC="../bootleg/bootlegs.gif" HEIGHT=39 WIDTH=220> </P></CENTER>
...
<HEAD>
   <TITLE>Bootlegs</TITLE>
      <META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=iso-8859-1">
         ...
         <BODY TEXT="#FFFFFF" BGCOLOR="#000000" LINK="#3333FF" VLINK="#33CCFF" ALINK="#FF0000">
<FONT SIZE=+3><B>ERROR!</B></FONT>
<P>why not try entering something to search for next time?</P>
         </BODY>
         </HTML>
